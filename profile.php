...
$survey_id = null; // Initialize $survey_id with a default value

if ($is_member) {
    // Query to retrieve the most recent pig survey
    $sql_pig_info = "SELECT id as survey_id, total_piglets, batch_count, pregnant_females, sows, boars, total_pigs
                     FROM pig_survey
                     WHERE userid = ?
                     ORDER BY id DESC
                     LIMIT 1"; // Fetch the most recent survey

    $stmt_pig_info = $conn->prepare($sql_pig_info);
    ...
    // Set default values if no survey exists
...
$batch_details = [];
if ($survey_id) {
    $sql_batch_details = "SELECT batch_number, piglet_count, birthdate 
                          FROM batch_details 
                          WHERE survey_id = ? AND piglet_count > 0";
    $stmt_batch_details = $conn->prepare($sql_batch_details);
...
$pregnancy_details = [];
if ($survey_id) {
    $sql_pregnancy_details = "SELECT female_number, pregnancy_date 
                              FROM pregnancy_details 
                              WHERE survey_id = ?";
    $stmt_pregnancy_details = $conn->prepare($sql_pregnancy_details);
    $stmt_pregnancy_details->bind_param("i", $survey_id);
...
// Set default values if no pregnancy details exist
if (empty($pregnancy_details)) {
    $pregnancy_details = [[
        'female_number' => 'N/A', // Default female number
        'pregnancy_date' => 'No Data Available', // Default pregnancy date
    ]];
}
    ...
    // Fetch aggregated totals for order items by order_id
$sql_aggregated_prices = "
    SELECT 
        order_id, 
...
$stmt = $conn->prepare($sql_aggregated_prices);
$stmt->execute();
$result = $stmt->get_result();

$order_totals = [];
while ($row = $result->fetch_assoc()) {
    $order_totals[$row['order_id']] = $row['total_price']; // Store total price by order_id
}
...
$sql_order_summary = "
    ...
    WHERE o.userid = ?
";
$stmt_summary = $conn->prepare($sql_order_summary);
$stmt_summary->bind_param("i", $userid);
$stmt_summary->execute();
$stmt_summary->bind_result($total_paid, $total_unpaid, $total_ordered);
$stmt_summary->fetch();
$stmt_summary->close();
...
$stmt_order_chart = $conn->prepare($sql_order_chart);
$stmt_order_chart->bind_param("i", $userid);
$stmt_order_chart->execute();
$result_order_chart = $stmt_order_chart->get_result();
...
$sql_count = "SELECT COUNT(DISTINCT o.order_id) AS total FROM `order` o WHERE userid = ?";
...
// Fallback to zero if any of the totals are NULL
...
// Query to retrieve order summary within the date range
$sql_totals = "
    SELECT 
        SUM(p.total_amount) AS total_ordered,
        SUM(p.amount_paid) AS total_paid,
        SUM(p.total_amount - p.amount_paid) AS total_unpaid
    FROM `order` o
...
$stmt->execute();
$stmt->bind_result($total_ordered, $total_paid, $total_unpaid);
$stmt->fetch();
$stmt->close();
...
@media screen and (max-width: 480px) {
    .profile-sidebar img {
        ...
        height: 60px; /* Ensure aspect ratio */
    }
    .profile-sidebar h2 {
        font-size: 16px; /* Smaller name text */
    }
    .profile-sidebar p {
        font-size: 12px; /* Smaller details text */
    }
    .tabs div {
        font-size: 12px; /* Even smaller font size for tabs */
        padding: 6px 8px; /* Reduce padding */
    }
    .chart-container {
        margin-bottom: 15px; /* Reduce spacing between charts */
    }
    .pagination a {
        font-size: 10px; /* Smaller pagination font */
        padding: 5px 8px; /* Compact pagination links */
    }
    .order-history-table th, .order-history-table td {
...
<tbody>
    <?php if (!empty($pregnancy_details)): ?>
        <?php foreach ($pregnancy_details as $pregnancy): 
            $pregnancyDate = $pregnancy['pregnancy_date'] !== 'No Data Available' 
                             ? new DateTime($pregnancy['pregnancy_date']) 
                             : null;
            $alert = ($pregnancyDate && $pregnancyDate->diff(new DateTime())->days >= 110) 
                     ? "⚠️ Soon to give birth" 
                     : ($pregnancyDate ? "Gestating" : "No Status");
        ?>
        <tr>
            ...
            <tbody>
                <?php if (!empty($batch_details)): ?>
                    <?php foreach ($batch_details as $batch): 
                        ...
                        $status = $ageInDays >= 150 ? "Ready to Sell" : "Not Ready";
                    ?>
                    <tr>
...
const orderSummaryChart = new Chart(ctxSummary, {
    type: 'bar',
    data: {
        labels: ['Total Ordered', 'Total Paid', 'Total Unpaid'],
        datasets: [{
            label: '₱',
            data: [
                <?php echo round($total_ordered, 2); ?>, 
                <?php echo round($total_paid, 2); ?>, 
                <?php echo round($total_unpaid, 2); ?>
            ],
